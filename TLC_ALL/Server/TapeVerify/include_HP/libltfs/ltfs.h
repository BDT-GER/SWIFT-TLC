...
#include "ltfs_locking.h"
#include "arch/signal_internal.h"
...
#define LTFS_BUILD_VERSION            19 	/* Build version */
...
struct dentry {
	...
	bool ondisk;             /**< True if this dentry is cached on disk, false if not */
...
struct ltfs_volume {
	...
	pthread_mutex_t reval_lock;    /**< Revalidation control. Must be pthread_mutex_t because of using pthread_cond_wait */
	pthread_cond_t reval_cond;
...
int ltfs_init(int log_level, bool use_syslog);
...
int ltfs_unmount(struct ltfs_volume *vol);
...
int ltfs_write_index(char partition, struct ltfs_volume *vol);
int ltfs_save_index_to_disk(const char *work_dir, bool need_gen, struct ltfs_volume *vol);
...
int ltfs_sync_index(struct ltfs_volume *vol, bool lock);
