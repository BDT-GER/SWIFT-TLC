...
namespace ltfs_management
{
	...
	{
	private:
		FormatThread(void* manager, const string& barcode, FormatType type, int priority, time_t startTime, bool bRequested = false);
	...
	private:
		...
		friend FormatThread* FormatManager::StartFormat(const string& barcode, FormatType type, int priority, Labels& labels, time_t startTimeInMicroSecs, bool bRequested);
	};
