...
namespace ltfs_management
{
    ...
    TapeLibraryMgr::TapeLibraryMgr(LtfsLibraries* hal, TapeDbManager* db, TapeSchedulerMgr* sch, TapeStateMachineMgr* stm, FormatManager* fmgr, CatalogDbManager* catalogDb)
    {
        ...
        autoRecycleFree_ = bdt::Factory::GetConfigure()->GetValueSize(bdt::Configure::AutoReformatFreePercent);
...
//#endif
    ...
    bool TapeLibraryMgr::NeedReformatTape(const TapeInfo& tapeInfo)
    {
    	if(tapeInfo.mFreeCapacity > (tapeInfo.mTotalCapacity * autoRecycleFree_ / 100) ){
    		LtfsLogDebug("NeedReformatTape: Tape " << tapeInfo.mBarcode << "'s free size " << tapeInfo.mFreeCapacity << " larger than config size " \
    				<< (tapeInfo.mTotalCapacity * autoRecycleFree_ / 100) << ". No need to reformat the tape.");
    		return false;
    	}
    	if(true == catalogDb_->TapeHasFile(tapeInfo.mGroupID, tapeInfo.mBarcode)){
    		LtfsLogDebug("NeedReformatTape: There are still files on tape " << tapeInfo.mBarcode);
    		return false;
    	}
    	LtfsLogInfo("NeedReformatTape: Tape " << tapeInfo.mBarcode << " should be re-formatted.");
    	return true;
    }
	...
	FormatThread* TapeLibraryMgr::StartFormat(const string& barcode, FormatType type, int priority, Labels& labels, time_t startTimeInMicroSecs, bool bRequested)
	{
		VS_DBG_LOG_FUNCTION;
		return formatMgr_->StartFormat(barcode, type, priority, labels, startTimeInMicroSecs, bRequested);
	}
